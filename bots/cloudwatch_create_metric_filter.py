...
def create_alarm(boto_session,filter_name,topic_arn):
  ...
  try:
      #Give the config somewhere to go
    alarm_name = f'{filter_name} Alarm'
...
def run_action(boto_session,rule,entity,params):
  ...
  topic_name = f"CloudTrailMetricFilterAlerts{region}"
  topic_arn = f'arn:aws:sns:{region}:{accountNumber}:{topic_name}'
  ...
  if log_group is None or log_group == "":
