...
def get_data_from_message(message):
    ...
    data['region'] = (
        data['region'].replace('_', '-') if data.get('region') else 'us-east-1'
    )
...
def handle_event(message, output_message):
    ...
    for bot_to_run in bots:
        ...
        if lambda_account_id == event_account_id:  # Boto will default to default session if we don't need assume_role credentials
            boto_session = boto3.Session(region_name=message_data.get('region'))

        elif account_mode == 'multi':  # multi or single account mode?
